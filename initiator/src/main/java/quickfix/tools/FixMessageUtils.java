package quickfix.tools;

import quickfix.*;
import quickfix.exception.QuickFixJException;
...
public class FixMessageUtils {
	...
	public static <T> List<T> findAnyGroup(Message message, Group group) {
        boolean exist=true;
        ...
        try {
            int i=1;
            while(exist) {
                item=(T)message.getGroup(i,group);
                list.add(item);
                i++;
            }
    ...
    public static List<List<Group>> findAllGroups(Message message, Group... groups) {
        List<List<Group>> list=new ArrayList<>();
        if(groups!=null){
            for(Group g:groups){
                list.add(findAnyGroup(message, g));
            }
        }
        return list;
    }
    ...
    public static void setFieldInGroup(Group group, FieldType fieldType, int tag, Object value) {
        Class<?> javaType = fieldType.getJavaType();
